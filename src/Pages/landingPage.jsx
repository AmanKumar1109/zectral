...
const T = {
  ...
  body: { fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 13, lineHeight: 1.9, color: 'rgba(255,255,255,0.6)' },
  card: { 
...
export default function LandingPage() {
  ...
  const onFocus = e => (e.target.style.borderColor = 'rgba(255,255,255,0.5)')
    ...
    <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <section style={{ position: 'relative', height: '100vh', zIndex: 10 }}>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', padding: 'clamp(16px,3vw,24px) clamp(16px,4vw,32px)' }}>
          ...
          <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
            <div ref={logoRef} style={{ display: 'flex', alignItems: 'center', gap: 10, color: '#fff', fontWeight: 700, fontSize: 13, letterSpacing: '0.28em', textTransform: 'uppercase', opacity: 0 }}>
              <span style={{ display:'flex',alignItems:'center',justifyContent:'center',width:22,height:22,fontSize:9,background:'rgba(255,255,255,.12)',border:'1px solid rgba(255,255,255,.22)',borderRadius:3,fontWeight:900 }}>Z</span>
              ZECTRAL
            </div>
            <div ref={navBtnsRef} style={{ display: 'flex', alignItems: 'center', gap: 12, opacity: 0 }}>
              {['PROJECTS','MENU'].map(label => (
                <button key={label} id={`${label.toLowerCase()}-btn`}
                  ...
                  style={{ display:'flex',alignItems:'center',gap:8,fontSize:10,letterSpacing:'0.2em',color:'rgba(255,255,255,.75)',padding:'8px 18px',border:'1px solid rgba(255,255,255,.18)',background:'rgba(255,255,255,.04)',backdropFilter:'blur(12px)',borderRadius:4,cursor:'pointer',textTransform:'uppercase' }}
                  onMouseEnter={e => gsap.to(e.currentTarget,{background:'rgba(255,255,255,.1)',color:'#fff',duration:.2})}
                  onMouseLeave={e => gsap.to(e.currentTarget,{background:'rgba(255,255,255,.04)',color:'rgba(255,255,255,.75)',duration:.2})}
                >
                  {label === 'PROJECTS' ? <span style={{color:'#fff',fontSize:9}}>✦</span>
                    : <svg style={{width:11,height:11,opacity:.7}} viewBox="0 0 24 24" fill="currentColor"><path d="M3 6h18v2H3zm0 5h18v2H3zm0 5h18v2H3z"/></svg>}
                  {label}
                </button>
              ))}
            </div>
          ...
          <footer ref={footerBarRef} style={{ display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0,opacity:0 }}>
            <button id="sound-btn" onClick={toggleSound}
              style={{ display:'flex',alignItems:'center',gap:8,color:'rgba(255,255,255,.38)',fontSize:10,letterSpacing:'0.18em',textTransform:'uppercase',background:'transparent',border:'none',cursor:'pointer' }}
              onMouseEnter={e=>gsap.to(e.currentTarget,{color:'rgba(255,255,255,.8)',duration:.2})}
              onMouseLeave={e=>gsap.to(e.currentTarget,{color:'rgba(255,255,255,.38)',duration:.2})}
            >
            ...
            <a href="#" id="chat-btn"
              style={{ display:'flex',alignItems:'center',gap:8,color:'rgba(255,255,255,.38)',fontSize:10,letterSpacing:'0.18em',textTransform:'uppercase',textDecoration:'none' }}
              onMouseEnter={e=>gsap.to(e.currentTarget,{color:'rgba(255,255,255,.8)',duration:.2})}
              onMouseLeave={e=>gsap.to(e.currentTarget,{color:'rgba(255,255,255,.38)',duration:.2})}
            >
              CHAT WITH US
        ...
        >
          {navLinks.map(({ label, active }) => (
            <a key={label} href="#" id={`side-nav-${label.toLowerCase()}`}
              style={{ display:'flex',alignItems:'center',gap:8,fontSize:9,letterSpacing:'0.22em',textTransform:'uppercase',color:active?'rgba(255,255,255,.95)':'rgba(255,255,255,.3)',fontWeight:active?600:400,textDecoration:'none',opacity:0 }}
              onMouseEnter={e => !active && gsap.to(e.currentTarget,{color:'rgba(255,255,255,.65)',x:3,duration:.2})}
              onMouseLeave={e => !active && gsap.to(e.currentTarget,{color:'rgba(255,255,255,.3)',x:0,duration:.2})}
            >
              {active
                ...
                : <span style={{width:1,height:12,background:'rgba(255,255,255,.18)',flexShrink:0}}/>
              }
              {label}
            </a>
          ))}
        ...
        <div style={{ position:'absolute',left:'50%',top:'52%',transform:'translate(-50%,-50%)',width:'100%',maxWidth:560,display:'flex',flexDirection:'column',alignItems:'center',textAlign:'center',gap:18,padding:'0 16px',boxSizing:'border-box' }}>
          <div ref={eyebrowRef} style={{ display:'flex',alignItems:'center',gap:12,opacity:0 }}>
            <span style={{width:32,height:1,background:'rgba(255,255,255,.25)'}}/>
            <span style={{fontSize:9,letterSpacing:'0.35em',color:'rgba(255,255,255,.45)',textTransform:'uppercase'}}>Est. 2024 · Enterprise Cloud Platform</span>
            <span style={{width:32,height:1,background:'rgba(255,255,255,.25)'}}/>
          </div>
          ...
          <h1 ref={headlineRef} style={{ ...T.heading,fontSize:'clamp(26px,4.5vw,48px)',textShadow:'0 0 60px rgba(255,255,255,.15),0 2px 20px rgba(0,0,0,.6)',opacity:0 }}>
            ORCHESTRATE YOUR<br/>
            <span style={{color:'rgba(255,255,255,.9)'}}>CLOUD INFRASTRUCTURE</span>
          </h1>

          <div ref={dividerRef} style={{width:40,height:1,background:'rgba(255,255,255,.2)',opacity:0}}/>
          ...
          <button ref={ctaBtnRef} id="discover-btn"
            style={{ display:'flex',alignItems:'center',gap:10,fontSize:10,letterSpacing:'0.25em',color:'rgba(255,255,255,.88)',textTransform:'uppercase',padding:'11px 28px',border:'1px solid rgba(255,255,255,.22)',background:'rgba(255,255,255,.05)',backdropFilter:'blur(12px)',borderRadius:3,cursor:'pointer',marginTop:4,opacity:0 }}
            onMouseEnter={e=>gsap.to(e.currentTarget,{background:'rgba(255,255,255,.1)',scale:1.03,duration:.25})}
            onMouseLeave={e=>gsap.to(e.currentTarget,{background:'rgba(255,255,255,.05)',scale:1,duration:.25})}
          >
          ...
          <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))',gap:'clamp(40px,6vw,72px)',alignItems:'center' }}>
            ...
            <div>
              <h2 style={{ ...T.heading,fontSize:'clamp(22px,3.5vw,42px)',marginBottom:24 }}>
                Deploy at the speed<br/><span style={{color:'rgba(255,255,255,.9)'}}>of thought</span>
              </h2>
              ...
              <button style={{ marginTop:32,display:'inline-flex',alignItems:'center',gap:10,fontSize:10,letterSpacing:'0.22em',color:'#fff',textTransform:'uppercase',padding:'10px 24px',border:'1px solid rgba(255,255,255,.3)',background:'rgba(255,255,255,.04)',borderRadius:3,cursor:'pointer' }}
                onMouseEnter={e=>gsap.to(e.currentTarget,{background:'rgba(255,255,255,.1)',borderColor:'rgba(255,255,255,.65)',duration:.2})}
                onMouseLeave={e=>gsap.to(e.currentTarget,{background:'rgba(255,255,255,.04)',borderColor:'rgba(255,255,255,.3)',duration:.2})}
              >
                EXPLORE FEATURES
            ...
            <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))',gap:16 }}>
              {stats.map((s, i) => (
                <div key={s.label} ref={el => statsRef.current[i] = el}
                  ...
                  onMouseLeave={e=>{e.currentTarget.style.borderColor='rgba(255,255,255,0.05)'; e.currentTarget.style.background='linear-gradient(135deg, rgba(255,255,255,0.02) 0%, rgba(255,255,255,0.002) 100%)'}}
                >
                  ...
                  <div style={{ fontSize:10,letterSpacing:'0.18em',color:'rgba(255,255,255,.4)',textTransform:'uppercase',fontWeight:500 }}>{s.label}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(270px,1fr))',gap:24 }}>
            {testimonials.map((t, i) => (
              <div key={t.name} ref={el => testiCards.current[i] = el}
                ...
                onMouseEnter={e=>gsap.to(e.currentTarget,{y:-6,background:'rgba(255,255,255,.05)',borderColor:'rgba(255,255,255,.18)',boxShadow:'0 12px 40px rgba(255,255,255,0.03)',duration:.35,ease:'power2.out'})}
                onMouseLeave={e=>gsap.to(e.currentTarget,{y:0,background:'linear-gradient(135deg, rgba(255,255,255,0.03) 0%, rgba(255,255,255,0.005) 100%)',borderColor:'rgba(255,255,255,.06)',boxShadow:'none',duration:.35,ease:'power2.out'})}
              >
                <div style={{fontSize:44,color:'#ffffff',opacity:.18,lineHeight:1,marginBottom:4,fontFamily:'Georgia,serif'}}>"</div>
                <p style={{ ...T.body,marginBottom:28,fontSize:13.5,lineHeight:1.8 }}>{t.quote}</p>
                <div style={{ display:'flex',alignItems:'center',gap:14,borderTop:'1px solid rgba(255,255,255,0.05)',paddingTop:18 }}>
                  <div style={{ width:38,height:38,borderRadius:'50%',background:'linear-gradient(135deg,rgba(255,255,255,.05),rgba(255,255,255,.15))',border:'1px solid rgba(255,255,255,.15)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:600,color:'#ffffff',letterSpacing:'0.05em',flexShrink:0 }}>
                    {t.initials}
                  </div>
                  <div>
                    <div style={{fontSize:12,fontWeight:600,color:'#fff',letterSpacing:'0.05em'}}>{t.name}</div>
                    <div style={{fontSize:10,color:'rgba(255,255,255,.4)',letterSpacing:'0.08em',marginTop:3}}>{t.role}</div>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))',gap:16 }}>
            {clients.map((c, i) => (
              <div key={c.name} ref={el => clientItems.current[i] = el}
                ...
                onMouseEnter={e=>{e.currentTarget.style.background='rgba(255,255,255,.04)'; e.currentTarget.style.borderColor='rgba(255,255,255,.18)'}}
                onMouseLeave={e=>{e.currentTarget.style.background='linear-gradient(135deg, rgba(255,255,255,0.02) 0%, rgba(255,255,255,0.002) 100%)'; e.currentTarget.style.borderColor='rgba(255,255,255,.05)'}}
              >
                ...
                <div style={{ fontSize:12, color:'rgba(255,255,255,.5)', lineHeight:1.4, fontFamily:"'Plus Jakarta Sans', sans-serif" }}>{c.desc}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:'clamp(40px,6vw,72px)' }}>
            ...
            <div>
              <h2 style={{ ...T.heading,fontSize:'clamp(20px,3vw,38px)',marginBottom:20 }}>
                Scale Your<br/><span style={{color:'rgba(255,255,255,.9)'}}>Infrastructure</span>
              </h2>
              ...
              {[
                ...
                <div key={item.label} style={{ marginBottom:24,display:'flex',alignItems:'flex-start',gap:16 }}>
                  ...
                  <div>
                    <div style={{fontSize:9,letterSpacing:'0.2em',color:'rgba(255,255,255,.3)',marginBottom:4,textTransform:'uppercase'}}>{item.label}</div>
                    <div style={{fontSize:13,color:'rgba(255,255,255,.7)',letterSpacing:'0.03em'}}>{item.value}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <form onSubmit={e => { e.preventDefault(); console.log('Form:', form) }}
              ...
              {[{ name:'name',label:'YOUR NAME',type:'text',placeholder:'Jane Doe' },
                ...
                <div key={f.name}>
                  <label style={{display:'block',fontSize:9,letterSpacing:'0.2em',color:'rgba(255,255,255,.35)',marginBottom:8,textTransform:'uppercase'}}>{f.label}</label>
              ...
              <div>
                <label style={{display:'block',fontSize:9,letterSpacing:'0.2em',color:'rgba(255,255,255,.35)',marginBottom:8,textTransform:'uppercase'}}>YOUR TEAM AND WORKFLOW</label>
              ...
              <button type="submit" style={{ marginTop:8,padding:'13px 0',width:'100%',background:'rgba(255,255,255,.08)',border:'1px solid rgba(255,255,255,.3)',borderRadius:3,color:'#ffffff',fontSize:10,letterSpacing:'0.25em',textTransform:'uppercase',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:10 }}
                onMouseEnter={e=>gsap.to(e.currentTarget,{background:'rgba(255,255,255,.18)',borderColor:'rgba(255,255,255,.7)',duration:.2})}
                onMouseLeave={e=>gsap.to(e.currentTarget,{background:'rgba(255,255,255,.08)',borderColor:'rgba(255,255,255,.3)',duration:.2})}
              >
                REQUEST DEMO
                <svg style={{width:12,height:12}} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
              </button>
      ...
      <footer id="footer" style={{ position:'relative',zIndex:10,background:'transparent',padding:'clamp(48px,8vw,72px) clamp(16px,5vw,48px) clamp(24px,4vw,36px)',borderTop:'1px solid rgba(255,255,255,.06)' }}>
        <div style={{ maxWidth:1200,margin:'0 auto' }}>
          <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:'clamp(32px,5vw,48px)',marginBottom:'clamp(40px,6vw,64px)' }}>
            ...
            {/* Brand */}
            <div>
              <div style={{ display:'flex',alignItems:'center',gap:10,color:'#fff',fontWeight:700,fontSize:13,letterSpacing:'0.28em',textTransform:'uppercase',marginBottom:14 }}>
                <span style={{display:'flex',alignItems:'center',justifyContent:'center',width:22,height:22,fontSize:9,background:'rgba(255,255,255,.1)',border:'1px solid rgba(255,255,255,.2)',borderRadius:3,fontWeight:900}}>Z</span>
                ZECTRAL
              </div>
              <p style={{fontSize:12,lineHeight:1.8,color:'rgba(255,255,255,.35)',maxWidth:220}}>
            ...
            {footerCols.map(col => (
              <div key={col.heading}>
                <div style={{fontSize:9,letterSpacing:'0.25em',color:'rgba(255,255,255,.3)',textTransform:'uppercase',marginBottom:18}}>{col.heading}</div>
                {col.links.map(link => (
                  <a key={link} href="#"
                    style={{display:'block',fontSize:12,color:'rgba(255,255,255,.45)',textDecoration:'none',marginBottom:10,letterSpacing:'0.03em'}}
                    onMouseEnter={e=>gsap.to(e.currentTarget,{color:'#fff',x:3,duration:.2})}
                    onMouseLeave={e=>gsap.to(e.currentTarget,{color:'rgba(255,255,255,.45)',x:0,duration:.2})}
                  >{link}</a>
                ))}
              </div>
            ))}
          ...
          <div style={{borderTop:'1px solid rgba(255,255,255,.06)',paddingTop:24,display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:12}}>
            <span style={{fontSize:10,color:'rgba(255,255,255,.22)',letterSpacing:'0.12em'}}>© 2024 ZECTRAL. ALL RIGHTS RESERVED.</span>
            <div style={{display:'flex',gap:24}}>
              {['Twitter','LinkedIn','Instagram'].map(s => (
                <a key={s} href="#"
                  style={{fontSize:10,color:'rgba(255,255,255,.28)',textDecoration:'none',letterSpacing:'0.12em',textTransform:'uppercase'}}
                  onMouseEnter={e=>gsap.to(e.currentTarget,{color:'#fff',duration:.2})}
                  onMouseLeave={e=>gsap.to(e.currentTarget,{color:'rgba(255,255,255,.28)',duration:.2})}
                >{s}</a>
              ))}
            </div>
          </div>
      ...
      >
        <button onClick={() => setMenuOpen(false)}
          style={{position:'absolute',top:32,right:32,display:'flex',alignItems:'center',gap:8,fontSize:10,letterSpacing:'0.2em',color:'rgba(255,255,255,.4)',background:'transparent',border:'none',cursor:'pointer',textTransform:'uppercase'}}
          onMouseEnter={e=>gsap.to(e.currentTarget,{color:'#fff',duration:.2})}
          onMouseLeave={e=>gsap.to(e.currentTarget,{color:'rgba(255,255,255,.4)',duration:.2})}
        >✕ CLOSE</button>
        ...
        <nav style={{display:'flex',flexDirection:'column',alignItems:'center',gap:24}}>
          {['HOME','PROJECTS','ABOUT US','CONTACT','CAREERS'].map((item, i) => (
            <a key={item} ref={el => menuItemsRef.current[i] = el} href="#"
              ...
              style={{fontSize:'clamp(2rem,5vw,3.5rem)',fontWeight:700,letterSpacing:'0.15em',color:'rgba(255,255,255,.65)',textTransform:'uppercase',textDecoration:'none',opacity:0}}
              onMouseEnter={e=>gsap.to(e.currentTarget,{color:'#fff',x:10,duration:.25})}
              onMouseLeave={e=>gsap.to(e.currentTarget,{color:'rgba(255,255,255,.65)',x:0,duration:.25})}
            >{item}</a>
          ))}
        </nav>

        <div style={{position:'absolute',bottom:32,display:'flex',gap:16,color:'rgba(255,255,255,.18)',fontSize:9,letterSpacing:'0.2em'}}>
          <span>© 2024 ZECTRAL</span>
          <span style={{width:1,height:10,background:'rgba(255,255,255,.2)'}}/>
